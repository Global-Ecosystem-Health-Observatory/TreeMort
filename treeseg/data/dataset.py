...
class DeadTreeDataset(Dataset):
    def __init__(self, root_dir, split='train', crop_size=256, transform=None, binarize=False):
    ...
    def center_crop_or_pad(self, image, label, size=256):
        ...
        if h < ch or w < cw:
            ...
            label = np.pad(label, ((pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2)), mode='constant', constant_values=0)
            h, w = image.shape[:2]
...
def prepare_datasets(root_dir, conf):
    ...
    full_train_dataset = DeadTreeDataset(root_dir, split='Train', crop_size=conf.train_crop_size, transform=train_transform, binarize=conf.binarize)
    test_dataset = DeadTreeDataset(root_dir, split='Test', crop_size=conf.test_crop_size, transform=test_transform, binarize=conf.binarize)
    ...
    val_loader = DataLoader(val_dataset, batch_size=conf.train_batch_size, shuffle=False, drop_last=True)
