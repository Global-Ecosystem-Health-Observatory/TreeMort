from treemort.training.train_loop import train_one_epoch_kd
from treemort.training.validation_loop import validate_one_epoch
from treemort.training.callback_handler import handle_callbacks

from treemort.utils.logger import get_logger
from treemort.utils.callbacks import EarlyStopping

logger = get_logger(__name__)


def trainer(
    student_model,
    teacher_model,
    optimizer,
    criterion,
    kd_criterion,
    metrics,
    train_loader,
    val_loader,
    conf,
    callbacks,
):
    device = next(student_model.parameters()).device

    teacher_model.eval()

    for epoch in range(conf.epochs):
        logger.info(f"Epoch {epoch + 1}/{conf.epochs} - Training started.")

        train_loss, train_metrics = train_one_epoch_kd(
            student_model, 
            teacher_model,
            optimizer, 
            criterion, 
            kd_criterion,
            metrics, 
            train_loader, 
            conf, 
            device, 
            alpha=conf.distillation_alpha,
            temperature=conf.distillation_temperature
        )

        logger.info(f"Epoch {epoch + 1} - Training completed.")
        logger.info(f"Training Loss: {train_loss:.4f}")
        logger.info(f"Training Metrics: {train_metrics}")

        val_loss, val_metrics = validate_one_epoch(student_model, criterion, metrics, val_loader, conf, device)

        logger.info(f"Epoch {epoch + 1} - Validation completed.")
        logger.info(f"Validation Loss: {val_loss:.4f}")
        logger.info(f"Validation Metrics: {val_metrics}")

        handle_callbacks(callbacks, epoch, student_model, optimizer, val_loss)

        if any([isinstance(cb, EarlyStopping) and cb.stop_training for cb in callbacks]):
            logger.info("Early stopping triggered.")
            break

    logger.info("Training process completed.")