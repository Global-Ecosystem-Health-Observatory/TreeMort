...
def _process_single_region(args):
    ...
    x = center[0] + a * np.cos(t) * np.cos(orientation) - b * np.sin(t) * np.sin(orientation)
    y = center[1] + a * np.cos(t) * np.sin(orientation) + b * np.sin(t) * np.cos(orientation)
    ...
    return {
        ...
        "properties": {
            ...
            "area": area,
            "ellipse_center": center_geo.tolist(),
            "ellipse_axes": axes,
